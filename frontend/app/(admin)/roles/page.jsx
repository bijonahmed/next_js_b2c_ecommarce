...
export default function UserPage() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE}/roles/delete/${id}`,
        {
          method: "DELETE",
          headers: {
  ...
  const fetchUsers = async (
    ...
    try {
      const url = `${process.env.NEXT_PUBLIC_API_BASE}/roles/index?page=${page}&pageSize=${pageSize}&searchQuery=${searchQuery}&selectedFilter=${selectedFilter}`;
  ...
  const columns = [
    ...
    { name: "Role Type", selector: (row) => row.type_name, sortable: true },
    {
      name: "Actions",
      cell: (row) => (
        <div className="d-flex gap-2">
          {perms.includes("edit role") ? (
            <button
              ...
              onClick={() => router.push(`/roles/edit/${row.id}`)}
            >
